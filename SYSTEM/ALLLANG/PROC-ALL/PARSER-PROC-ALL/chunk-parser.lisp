               ...
               ; *** it is a nominative pron, and we are not in case of English inversion
                 (and (eq w-categ 'PUNCT) (eq w-lemma #\,)				;[10]
                      (or (and (eq w-n-categ 'ADV) (eq w-n2-categ 'VERB))
                          ...
                          (and (is-a-synt-noun-complex nxtline allines allinks)
...
(defun choose-best-assign (verbline unlinked cf-match allines allinks)
  (let* ((best (first cf-match))
   ...
   ;(break "")
     	      (cond ((and (is-verb-sem-compatible verbline unlinked dnext allines allinks)
...
;     >>> dnext is the candidate assignment
...
(defun is-verb-sem-compatible (verbline unlinked dnext allines allinks)
  (let ((vsem (get-word-meaning (get-synt-word verbline) 'VERB nil nil))
        (cases (second (first dnext)))
        nheads nmarks noungrouphead fail them-grid sel-restr dep-mean)
  ...
  ; *** in case of absence of the thematic grid, the assignment is accepted
          (t (setq them-grid (rest (second (first vsem))))
    ...
    ;(break "")
             (do ((nxthead (first nheads) (first nheads))
                ...
                ((or (null nxthead) fail)
                 ;  (format t "end of check: ~a ~%" fail)
                 ;  (break "")
                   (not fail))
                (setq sel-restr (first (leggi them-grid nxtmark)))
                (setq dep-mean (get-word-meaning (get-synt-word nxthead) 
                                                 (get-synt-categ nxthead) 
                                                 (get-synt-type nxthead) nil))
        ;(format t "Next selection restriction: ~a~%Next word meaning: ~a~%  " sel-restr dep-mean)
        ;(break "")
                (cond ((not (is-instance-or-subclass-of (first dep-mean) (first (get sel-restr 'range))))
                         (setq fail t))))))))

; **************************************************************************
; *** the criteria for being a best match are:
;	1. A 'verbal-locution' label is assigned
;       1bis. There is no unmarked non-temporal RMOD
 ...
 (case ind
  (1 (let ((locut1 (locut-member (second match1)))
       ...
       ; *** obj-locut or in both or in none (parity wrt criterium 1)
             (t 'fair))))
  ;(2 (let ((unm-rmod1 (with-unmarked-rmod (second match1) unlinked))
  ;         (unm-rmod2 (with-unmarked-rmod (second match2) unlinked)))
  ;     ; *** unmarked rmod only in match1 (CRITERIUM 2) **************************************
;	(cond ((and unm-rmod1 (not unm-rmod2)) nil)
;       ; ***  unmerked rmod only in match2 (inverse of criterium 2)
...
; *****************************************************************************
(defun with-unmarked-rmod (labels cases)
   (cond ((null labels) nil)
         ((lab-subsumes 'RMOD (first labels))
            (cond ((and (is-a-synt-noun-complex (first cases) allines allinks)
                     ...
                     t)
                  (t (with-unmarked-rmod (rest labels) (rest cases)))))
         (t (with-unmarked-rmod (rest labels) (rest cases)))))
